...
BEGIN
    ...
    BEGIN TRY
		...
		-- Insert transformed product information from bronze to silver layer
		...
		SELECT
			...
			prd_nm,
			ISNULL(prd_cost, 0) AS prd_cost,                        -- Handle null costs with default 0
			CASE 
			...
			) AS DATE) AS prd_end_dt                                -- Set end date as one day before next product's start
